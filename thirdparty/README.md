...
- Version: 2.6.4 (2691aff4304a6d7e053199c205620136481b9dd1, 2024)
- License: MIT

Used by OpenColorIO to parse the CLF/CTF and Iridas Look XML file formats.
...
- `lib/expat_config.h` is generated by `modules/ocio/godot_update_opencolorio.py`
  in place of the one autotools/CMake would produce.
...
- Upstream: https://github.com/AcademySoftwareFoundation/Imath
- Version: 3.1.12 (c0396a055a01bc537d32f435aee11a9b7ed6f0b5, 2024)
- License: BSD-3-Clause

Only the `half` type is vendored; OpenColorIO uses nothing else from Imath.

Files extracted from upstream source:

- `src/Imath/{half.h,half.cpp,halfLimits.h}`
- `src/Imath/{ImathExport.h,ImathNamespace.h,ImathPlatform.h}`
- `LICENSE.md`

Patches:

- `Imath/ImathConfig.h` is generated by `modules/ocio/godot_update_opencolorio.py`
  from `config/ImathConfig.h.in`. `IMATH_HALF_NO_LOOKUP_TABLE` is defined by the
  module's SCsub so that the 128 KB half-to-float table (`toFloat.h`, itself
  generated upstream) is not needed; conversion uses the bit-shift path instead.
...
- Version: 2.4.2 (6918fad3f5d22ac3ef2397c754bf4268c2b58dd0, 2025)
- License: BSD-3-Clause

Files extracted from upstream source:

- `include/OpenColorIO/*.h`
- `src/OpenColorIO/**/*.{h,cpp}` (excluding `res/` and the `.ocio` config files)
- `src/utils/*.h`
- `ext/xxHash/src/include/` and `ext/sampleicc/src/include/` as `ext/*/include/`,
  the two header-only libraries upstream bundles
- `LICENSE`, `THIRD-PARTY.md`

Patches:

- `src/OpenColorIO/OCIOZArchive.cpp` is replaced by a stub. It is the only file
  that depends on minizip-ng, and it exists solely to read and write `.ocioz`
  archived configs, which Godot does not support. Every entry point fails with a
  clear message. To restore archive support, vendor minizip-ng and take the file
  from upstream unmodified.
- `include/OpenColorIO/OpenColorABI.h`, `src/OpenColorIO/CPUInfoConfig.h` and
  `generated_include/{CG,Studio}.cpp` are produced by
  `modules/ocio/godot_update_opencolorio.py` in place of the CMake configure
  step. The last two hold OpenColorIO's built-in ACES configs, hex-encoded from
  `src/OpenColorIO/builtinconfigs/configs/*.ocio`.
- SIMD is left off (`OCIO_USE_SSE2` and friends are 0 in `CPUInfoConfig.h`), so
  `ext/sse2neon` is not needed on arm64. The vectorised LUT kernels only matter
  for bulk CPU image processing.

The `modules/ocio/godot_update_opencolorio.py` script pulls this library and its
Imath, yaml-cpp, pystring and expat dependencies from upstream and applies the
above automatically.
...
- Upstream: https://github.com/imageworks/pystring
- Version: 1.1.4 (7d16bc814ccb4cad03c300dcb77440034caa84f7, 2022)
- License: BSD-3-Clause

Used by OpenColorIO for string and path manipulation.
...
- Version: 0.8.0 (f7320141120f720aecc4c32be25586e7da9eb978, 2023)
- License: MIT

Used by OpenColorIO to parse `.ocio` config files.

Files extracted from upstream source:

- `include/yaml-cpp/**/*.h`
- `src/**/*.{h,cpp}` (excluding `src/contrib/`)
- `LICENSE`
