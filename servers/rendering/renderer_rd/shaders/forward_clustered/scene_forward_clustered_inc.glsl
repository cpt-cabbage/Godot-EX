...
struct ImplementationData {
	...
	uint rt_gi; // Nonzero: indirect lighting comes from the ray-traced GI buffers. Bits 0-1 the resolution (1 full, 2 half), 4 the reflection buffer is populated, 8 directional, 16 specular occlusion from the bent normal, 32 reflection probe refit, 64 the mirror path (every roughness traced, SSR skipped); see RenderForwardClustered::_setup_environment.
...
// The half-resolution direct pass's analytic terms (unshadowed; the whole
// cell and the planar mirrors' image lights' share of it), for the
// per-pixel composite (stochastic_direct_lights == 2).
layout(set = 1, binding = 53) uniform texture2DArray stochastic_analytic_diffuse_buffer;
...
layout(set = 1, binding = 53) uniform texture2D stochastic_analytic_diffuse_buffer;
...
// Texture streaming material feedback buffer access (upstream binds this at
// 38; the ray tracing textures hold 38-51 here, so it follows them).
layout(set = 1, binding = 52, std430) buffer restrict MaterialFeedbackBuffer {
	uint data[];
}
material_feedback;
#endif
